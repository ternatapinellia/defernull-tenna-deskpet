...
# 导入对话配置
from dialogues import (DIALOG_START, DIALOG_CLOSE, DIALOG_CLICK,
                       DIALOG_RANDOM, DIALOG_FLIRT, DIALOG_DRINK,
                       DIALOG_LUNCH, DIALOG_DINNER, DIALOG_SLEEP)

# ---------- NewsAPI 配置 ----------
NEWSAPI_KEY = "1ddcd0c9296e438ba790d987e38fc6b7"
NEWSAPI_BASE_URL = "https://newsapi.org/v2/everything"

# ---------- 午夜新闻关键词 ----------
MIDNIGHT_KEYWORDS = ["凶杀", "案件", "犯罪", "枪击", "暴力", "谋杀", "命案", "死亡", "性侵"]
...
# ---------- 新闻API ----------
def fetch_news_from_api(keyword=None, use_search_api=False):
    """
    获取新闻
    keyword: 关键词列表，用于过滤
    use_search_api: 是否使用NewsAPI搜索（用于午夜新闻）
    """
    global _news_cache, _cache_time, _midnight_cache, _midnight_cache_time

    current_time = time.time()
    
    # 如果使用搜索API且有缓存
    if use_search_api and _midnight_cache is not None and (current_time - _midnight_cache_time) < CACHE_DURATION:
        print("使用缓存的午夜新闻数据")
        return _midnight_cache.copy()

    # 如果使用搜索API（午夜新闻专用）
    if use_search_api and keyword:
        return fetch_crime_news_from_newsapi(keyword)

    # 普通新闻缓存
    if _news_cache is not None and (current_time - _cache_time) < CACHE_DURATION:
        print("使用缓存的新闻数据")
        if keyword:
            filtered = [item for item in _news_cache if any(kw.lower() in item.get("title", "").lower() for kw in keyword)]
            return filtered
    ...
    # 如果指定了关键词，进行过滤
    if keyword:
        filtered_news = [item for item in raw_news if any(kw.lower() in item.get("title", "").lower() for kw in keyword)]
        print(f"关键词匹配到 {len(filtered_news)} 条新闻")
        _news_cache = filtered_news
...
def fetch_crime_news_from_newsapi(keywords):
    """
    使用 NewsAPI 搜索犯罪相关新闻（午夜新闻专用）
    """
    global _midnight_cache, _midnight_cache_time
    
    # 构建搜索查询（用 OR 连接关键词，中英文混合搜索）
    search_terms = ["凶杀", "命案", "谋杀", "犯罪", "枪击", "暴力", "homicide", "murder", "crime", "shooting", "violence"]
    query = " OR ".join(search_terms)
    encoded_query = urllib.parse.quote(query)
    
    # 按相关性排序，获取中文新闻
    url = f"{NEWSAPI_BASE_URL}?q={encoded_query}&sortBy=relevancy&language=zh&pageSize=30&apiKey={NEWSAPI_KEY}"
    
    try:
        print(f"正在从 NewsAPI 搜索犯罪新闻...")
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})
        with urllib.request.urlopen(req, timeout=15) as response:
            ...
            if data.get("status") == "ok":
                ...
                if articles:
                    ...
                    print(f"NewsAPI 搜索成功，共 {len(news_list)} 条结果")
                    _midnight_cache = news_list
                    _midnight_cache_time = time.time()
                    return news_list
                else:
                    print("NewsAPI 没有找到相关新闻")
            else:
                print(f"NewsAPI 返回错误: {data.get('message', '未知错误')}")
    except Exception as e:
        print(f"NewsAPI 请求失败: {e}")
    
    # NewsAPI 失败，回退到本地关键词过滤
    print("回退到本地关键词过滤...")
    return fetch_news_from_api(keywords, use_search_api=False)
...
class BubbleWidget(QWidget):
    def __init__(self, parent=None):
        ...
        self.big_font = QFont("Microsoft YaHei", int(60 * scale))
...
class BasePanel(QWidget):
    def __init__(self, parent=None, panel_type="panel"):
        ...
        self.bg_pixmap = QPixmap(resource_path("panel.png"))
...
class NewsWindow(BasePanel):
    def __init__(self, parent=None):
        ...
        self.bg_pixmap = QPixmap(resource_path("panel2.png"))
        scaled_bg = self.bg_pixmap.scaled(self.panel_width, self.panel_height, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        self.bg_pixmap = scaled_bg
        self.setMask(self.bg_pixmap.mask())
        ...
        # 标题
        title = QLabel("今日新闻", container)
        title.setStyleSheet(f"color: #1a1a2c; font-size: {int(24 * self.scale)}px; font-weight: bold; background: transparent;")
        title.setGeometry(int(60 * self.scale), int(60 * self.scale), int(330 * self.scale), int(40 * self.scale))
        ...
        """)
        refresh_btn.setGeometry(int(380 * self.scale), int(15 * self.scale), int(40 * self.scale), int(40 * self.scale))
        refresh_btn.clicked.connect(self.refresh_news)
        refresh_btn.setToolTip("刷新新闻")

        self.news_scroll = QScrollArea(container)
        ...
        self.loading_label = QLabel("正在加载新闻...", container)
        ...
        self.retry_btn = QPushButton("重新加载", container)
    ...
    def use_fallback_data(self):
        self.news_data = [
            ...
            {"title": "我国新能源汽车产量突破2000万辆，市场占有率持续提升", "url": "https://www.gov.cn"},
            {"title": "人工智能大模型发展迅速，多家企业发布新产品", "url": "https://www.gov.cn"},
            {"title": "全国夏粮再获丰收，总产量创历史新高", "url": "https://www.gov.cn"},
            {"title": "中国-东盟自贸区升级版谈判取得积极进展", "url": "https://www.gov.cn"},
            {"title": "国家发改委出台措施促进消费持续恢复", "url": "https://www.gov.cn"},
            {"title": "我国数字经济规模突破50万亿元", "url": "https://www.gov.cn"},
            {"title": "全国统一大市场建设加快推进，营商环境持续优化", "url": "https://www.gov.cn"},
            {"title": "中国科学家在量子计算领域取得重大突破", "url": "https://www.gov.cn"},
            {"title": "乡村振兴战略深入实施，农村居民收入稳步增长", "url": "https://www.gov.cn"},
            {"title": "国家卫健委发布最新健康中国行动实施方案", "url": "https://www.gov.cn"},
            {"title": "我国5G基站总数突破400万，网络覆盖持续完善", "url": "https://www.gov.cn"},
            {"title": "中国-中亚天然气管道累计输气量突破5000亿立方米", "url": "https://www.gov.cn"},
            {"title": "全国城镇新增就业完成全年目标的70%以上", "url": "https://www.gov.cn"},
        ]
        self.loading_label.hide()
        self.retry_btn.hide()
        self.current_page = 0
...
class MidnightNewsWindow(BasePanel):
    def __init__(self, parent=None):
        ...
        self.page_size = 5
        self.is_loading = False
        self.midnight_keywords = ["凶杀", "案件", "犯罪", "枪击", "暴力", "谋杀", "命案", "死亡", "性侵"]
        ...
        self.loading_label = QLabel("正在加载午夜新闻...", container)
        self.loading_label.setStyleSheet(f"color: #666; font-size: {int(18 * self.scale)}px; background: transparent;")
        self.loading_label.setGeometry(int(60 * self.scale), int(200 * self.scale), int(330 * self.scale), int(50 * self.scale))
        self.loading_label.setAlignment(Qt.AlignCenter)
    ...
    def _do_fetch_news(self):
        try:
            # 使用 NewsAPI 搜索犯罪新闻
            news_data = fetch_news_from_api(self.midnight_keywords, use_search_api=True)
            if news_data and len(news_data) > 0:
                self.news_data = news_data
                self.loading_label.hide()
                self.retry_btn.hide()
...
class ControlPanel(BasePanel):
    def __init__(self, parent=None):
        ...
        title = QLabel("控制面板", container)
        title.setStyleSheet(f"color: #1a1a2c; font-size: {int(28 * self.scale)}px; font-weight: bold; background: transparent;")
        title.setGeometry(int(60 * self.scale), int(60 * self.scale), int(330 * self.scale), int(40 * self.scale))

        group1 = QGroupBox("番茄钟", container)
        ...
        self.tomato_reset_btn = QPushButton("⟳ 重置")
        ...
        group2 = QGroupBox("提醒设置", container)
        ...
        sleep_h_layout.addWidget(QLabel("时间："))
        sleep_h_layout.addWidget(self.sleep_hour)
        sleep_h_layout.addWidget(QLabel("时"))
        sleep_h_layout.addWidget(self.sleep_min)
        sleep_h_layout.addWidget(QLabel("分"))
        layout2.addLayout(sleep_h_layout, 1, 1)

        self.eat_label = QLabel("午饭：12点  |  晚饭：18点")
    ...
    def update_tomato(self):
        if self.tomato_remaining > 0:
            ...
            """)
            self.parent_pet.stop_tomato_display()
            self.parent_pet.add_dialog(random.choice(DIALOG_DRINK))
            self.update_tomato_display()
    ...
    def check_reminders(self):
        ...
        if hour == 12 and minute == 0 and not self.lunch_triggered:
            self.parent_pet.add_dialog(random.choice(DIALOG_LUNCH))
            self.lunch_triggered = True
        if hour != 12:
            self.lunch_triggered = False

        if hour == 18 and minute == 0 and not self.dinner_triggered:
            self.parent_pet.add_dialog(random.choice(DIALOG_DINNER))
        ...
        if self.sleep_check.isChecked():
            ...
            if hour == set_h and minute == set_m and not self.sleep_triggered:
                self.parent_pet.add_dialog(random.choice(DIALOG_SLEEP))
...
class DesktopPet(QWidget):
    def __init__(self):
        ...
        self.menu.addAction("控制面板", self.show_control_panel)
        self.menu.addAction("调情", self.flirt)
        self.menu.addAction("今日新闻", self.show_news)
        self.menu.addAction("午夜新闻", self.show_midnight_news)
        self.toggle_animation_action = QAction("⏸ 暂停动画", self)
        ...
        self.add_dialog(random.choice(DIALOG_START))
    ...
    def create_tray_icon(self):
        ...
        self.tray_icon.setToolTip("桌宠 - 点击显示/隐藏")
        tray_menu = QMenu()
    ...
    def closeEvent(self, event):
        event.ignore()
        self.hide()
        self.tray_icon.showMessage("桌宠", "程序已最小化到系统托盘", QSystemTrayIcon.Information, 2000)
    ...
    def toggle_animation(self):
        ...
        if self.animation_paused:
            ...
            self.toggle_animation_action.setText("▶ 开始动画")
        else:
            self.animation_timer.start(300)
            self.current_frame_index = 0
            self.current_pixmap = self.pet_frames[0]
            self.label.setPixmapWithFade(self.current_pixmap)
            self.toggle_animation_action.setText("⏸ 暂停动画")
    ...
    def on_pet_click(self):
        self.add_dialog(random.choice(DIALOG_CLICK))

    def flirt(self):
        self.add_dialog(random.choice(DIALOG_FLIRT))

    def random_dialog(self):
        if not self.tomato_display_active and not self.is_displaying:
            self.add_dialog(random.choice(DIALOG_RANDOM))
    ...
    def quit_app(self):
        if hasattr(self, 'tray_icon'):
            self.tray_icon.hide()
        self.add_dialog(random.choice(DIALOG_CLOSE))
